<?php
	class Main_model extends Cms_Base_model {
		...
		public function general() {
			...
			// $data['logo_ads'] = $this -> ads_model -> Ads_get_for_show(1);
			...
			// var_dump($data['acreage_under']);die();
			...
			// $data['support_info'] = $this -> categories_news_model -> Get_child_news(16);
			// $data['company_info'] = $this -> categories_news_model -> Get_child_news(19);
			// $data['support_contact'] = $this -> news_model -> Get_details_by_id(10);

			// $data['menu_cate'] = $this -> categories_product_model -> Category_get_all_for_menu();
			// $data['menuResponsive_cate'] = $this -> categories_product_model -> CategoryResponsive_get_all_for_menu();
		...
		function trim_text($input, $length, $ellipses = true) {
			if($length=='full') {
				...
				//add ellipses (...)
				$trimmed_text = preg_replace("/<img[^>]+\>/i", " ", $trimmed_text); 
				$trimmed_text = str_replace('<p style="text-align: center;">&nbsp; </p>', "", $trimmed_text);
				if ($ellipses) {
				$trimmed_text .= '...';
				}
			}
