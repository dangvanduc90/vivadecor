<?php 
	class Ajax_news_categories_model extends Cms_Base_model {
		...
		public function Ajax_Update_IsCool() {
			$id = $this -> input -> post('id');
			$this -> input -> post('IsCool') ? $IsCool = 1 : $IsCool = 0;
			if($id !== false) {
				$data = array('IsCool' => (int)$IsCool,
								'ModifiedBy' => $this -> session -> userdata('userid'),
								'ModifiedDate' => date('Y-m-d H:i:s', time())
							);
		...
		public function Ajax_Check_Title_Except() {
			...
			if ($query -> num_rows() > 0) {
				...
				$this -> db -> from('categoriesnews');
				$this -> db -> where('Title', $title);
				$this -> db -> where_not_in('CategoriesNewsID', $id);
				$query2 = $this -> db -> get();
				if ($query2 -> num_rows() > 0) {
					return true;
				} else {
