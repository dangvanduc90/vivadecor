<?php 
	class Ajax_products_categories_model extends Cms_Base_model {
		...
		function Ajax_Update_Orders() {
			...
			if($id && $orders) {
				...
				$this -> db -> update('categoriesproducts',$data,array('CategoriesProductsID' => $id));
		...
		public function Ajax_Check_Title_Except() {
			...
			if ($query -> num_rows() > 0) {
				...
				$this -> db -> select('CategoriesProductsID');
				$this -> db -> from('categoriesproducts');
				$this -> db -> where_not_in('CategoriesProductsID', $id);
				$this -> db -> where('Title', $title);
				$query2 = $this -> db -> get();
				if ($query2 -> num_rows() > 0) {
					return true;
