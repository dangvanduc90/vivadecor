	...
	class Categories_news_model extends Cms_Base_model {
		...
		private function Categories($parentid = 0,$array = array(),$cateNewsActive = false){
			...
			if($parentid == 0) {
				...
				$categories .= ' ><a href="'.base_url().'">Home</a><ul style="height:36px;"></ul></li>';
			}
		...
		function Get_details($slug){
			global $lang;
			if($lang == 'en'){
				$this -> db -> select('categoriesnews.CategoriesNewsID,categoriesnews.Title_en as Title,categoriesnews.Slug,categoriesnews.CategoriesNewsID,categoriesnews.SEOTitle,categoriesnews.SEOKeyword,categoriesnews.SEODescription,categoriesnews.Banner,categoriesnews.ImagePreset');
			} else if($lang == 'fr'){
				$this -> db -> select('categoriesnews.CategoriesNewsID,categoriesnews.Title_fr as Title,categoriesnews.Slug,categoriesnews.CategoriesNewsID,categoriesnews.SEOTitle,categoriesnews.SEOKeyword,categoriesnews.SEODescription,categoriesnews.Banner,categoriesnews.ImagePreset');
			} else {
				$this -> db -> select('categoriesnews.CategoriesNewsID,categoriesnews.Title,categoriesnews.Slug,categoriesnews.CategoriesNewsID,categoriesnews.SEOTitle,categoriesnews.SEOKeyword,categoriesnews.SEODescription,categoriesnews.Banner,categoriesnews.ImagePreset');
			}
			...
			if ($query -> num_rows() > 0){
				$result = $query -> row();
				$result -> Banner = $result -> ImagePreset . $result -> Banner;
				return $result;
			} else {
				return false;
			}
		...
		function Get_details_by_id($id){
			global $lang;
			if($lang == 'en'){
				$this -> db -> select('categoriesnews.CategoriesNewsID,categoriesnews.Title_en as Title,categoriesnews.Slug,categoriesnews.SEOTitle,categoriesnews.SEOKeyword,categoriesnews.SEODescription,categoriesnews.Banner,categoriesnews.ImagePreset');
			} else if($lang == 'fr'){
				$this -> db -> select('categoriesnews.CategoriesNewsID,categoriesnews.Title_fr as Title,categoriesnews.Slug,categoriesnews.SEOTitle,categoriesnews.SEOKeyword,categoriesnews.SEODescription,categoriesnews.Banner,categoriesnews.ImagePreset');
			} else {
				$this -> db -> select('categoriesnews.CategoriesNewsID,categoriesnews.Title,categoriesnews.Slug,categoriesnews.SEOTitle,categoriesnews.SEOKeyword,categoriesnews.SEODescription,categoriesnews.Banner,categoriesnews.ImagePreset');
			}
			$this -> db -> from('categoriesnews');
			$this -> db -> where('categoriesnews.Publish', 1);
		...
		function Get_cate_and_detail_news($cate, $limit = false, $isHot = false){
			global $lang;
			if($lang == 'en'){
				$this -> db -> select('categoriesnews.CategoriesNewsID,categoriesnews.Title_en as Title,categoriesnews.Slug');
			} else if($lang == 'fr'){
				...
				$this -> db -> select('categoriesnews.CategoriesNewsID,categoriesnews.Title,categoriesnews.Slug,categoriesnews.Body');
			}
			$this -> db -> from('categoriesnews');
			$this -> db -> where('categoriesnews.Publish', 1);
			$this -> db -> where('categoriesnews.CategoriesNewsID', $cate);
			$this -> db -> order_by("Orders","asc");
			$query = $this -> db -> get();
			$parents = $query -> result();
			foreach($parents as $par){
				$par -> news = $this -> Get_cate_news($par -> CategoriesNewsID,$limit, $isHot);
			}
			return $parents;
		}

		public function Get_cate_news($cate, $limit = false, $isHot = false) {
			$this -> db -> select("Title, Slug, CreatedDate, ImageURL, ImagePreset, ImageAlt, ImageTitle,Body");
			$this -> db -> where("Publish",1);
			if ($cate) {
				$this -> db -> where("CategoriesNewsID", $cate);
			}
			if ($limit) {
				$this -> db -> limit($limit);
			}
			if ($isHot) {
				$this -> db -> where("IsHot", $isHot);
			}
			...
			foreach ($result as $re) {
				$re -> CreatedDate = date("d/m/Y",strtotime($re -> CreatedDate)) ;
