<?php
	...
	class Menu_model extends Cms_Base_model {
		...
		private function Menu($parentid = 0,$array = array(),$menuActive = false){
			...
			if(count($temp) > 0) {
				if ($parentid != 0) {
					$menu .= '<ul class="ampstart-dropdown-items list-reset m0 p0">';
				}
				for ($i=0; $i < count($temp); $i++) {
					if($this -> checkHasChild($temp[$i]['MenuID'])){
							$menu .= '<ul class="list-reset m0 p0 caps h5">';
							$menu .= '<li class="ampstart-nav-item ampstart-nav-dropdown relative">';
							$menu .= '<amp-accordion layout="container" disable-session-states="" class="ampstart-dropdown">';
							$menu .= '<section>';
							$menu .= '<header>';
							$menu .= $temp[$i]["Title"];
							$menu .= '</header>';
						...
						if ($temp[$i]['ParentID'] == 0) {
							$menu .= '<ul class="ampstart-sidebar-faq list-reset m0">';
							$menu .= '<li class="ampstart-faq-item"><a href="';
							if ($temp[$i]['Url'] == "") {
								$menu .= base_url() . $temp[$i]['Url'];
							}else
								$menu .= base_url() . $temp[$i]['Url'].'.html';
							$menu .= '" class="text-decoration-none">';
							$menu .= $temp[$i]["Title"];
							$menu .= '</a>';
							$menu .= '</li>';
							$menu .= '</ul>';
							...
							$menu .= '<li class="ampstart-dropdown-item"><a href="';
							if ($temp[$i]['Url'] == "") {
								$menu .= base_url() . $temp[$i]['Url'];
							}else
								$menu .= base_url() . $temp[$i]['Url'].'.html';
			...
			if(count($temp) > 0) {
				...
				for ($i=0; $i < count($temp); $i++) {
					...
					if( $this -> checkHasChild($temp[$i]['MenuID'])) {
						...
						$menu .= ' class="dropdown" >';
						$menu .= '<a class="dropdown-toggle" data-toggle="dropdown" href="#">'.$temp[$i]['Title'].'<span class="caret"></span></a>';
					...
					$menu .= $this -> MenuResponsive($temp[$i]['MenuID'],$array);
					$menu .= '</li>';
