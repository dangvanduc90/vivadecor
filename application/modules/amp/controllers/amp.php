<?php
	...
	class Amp extends CMS_BaseController {
...

		{
			...
			$this->config->set_item('base_url','https://amp.vivadecor.com.vn') ;
		...
		function error_404(){
			...
			$this -> load -> view('amp/error_404', $data);
		...
		function trang_chu($message = false){
			...
			$this -> load -> view('amp/main_index', $data);
		...
		function products($slug = "") {
			...
			if (!$slug) {
				...
				$this -> load -> view('amp/main_product1.php', $data);
				...
				$data['signature'] = $this -> news_model -> News_get_by_cate_id(3);
				...
				$this -> load -> view('amp/main_product2.php', $data);
		...
		function news($slug = ""){
			...
			if(!$slug){
				...
				$this -> load -> view('amp/main_news1.php', $data);
				...
				$this -> load -> view('amp/main_news2.php', $data);
		...
		function news_tags($tags){
			...
			if(!$tags){
				...
				$this -> load -> view('amp/main_tag_news.php', $data);
...
// var_dump($config['base_url']);
			...
			$this -> load -> view('amp/main_search.php', $data);
		...
		function products_tags($tags){
			...
			if(!$tags){
				...
				$this -> load -> view('amp/main_tag_products.php', $data);
