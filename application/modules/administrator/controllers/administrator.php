...
class Administrator extends CMS_AdminController
{
    ...
    public function convert_db()
    {
        ...
        $allChildId = $this->categories_news_model->convert();
        if ($allChildId) {
            var_dump($allChildId);
        } else echo "123";
    }
    ...
    public function seolink($action = false, $id = false)
    {
        $this->load->model("seo_link_model");
        if ($action !== false) {
            if ($action == "save") {
                if ($this->seo_link_model->Update_SEO_Link($id)) {
                    redirect(base_url() . "administrator/seolink");
                } else {
                    echo 'fail';
                }
            } elseif ($action == "add") {
                ...
                if (!$this->form_validation->run()) {
                    $this->load->view('includes/header', $data);
                    $this->load->view('seo_link_add_view');
                    $this->load->view('includes/footer');
                } else {
                    if ($this->seo_link_model->insert()) {
                        redirect(base_url() . 'administrator/seolink');
                    } else {
                        echo "fail";
                    }
            ...
            $data['seolinks'] = $this->seo_link_model->Get_All_SEO_Link();
            $data['js'] = '<script>$(function() {$( "#tabs" ).tabs();});</script>';
            $this->load->view('includes/header', $data);
            $this->load->view('seo_link_view');
            $this->load->view('includes/footer');
        }
    }
    /************************************************************************************************/
    ...
    /*********** Ben duoi la cac method thao tac voi quan ly sections
     ************************************************************************************************/

    public function sections($action = false, $id = false)
    {
        $data['js'] = '<script>$(function() {$( "#tabs" ).tabs();});</script>';
        if ($action !== false) {
            switch ($action) {
                case 'edit' :
                    {
                        if ($id !== false) {
                            ...
                            $section = $this->Section_model->Section_get_by_id($id);
                            $data['section'] = $section->row_array();
                            // thuc hien khi form edit product duoc submit len server
                            if (!$this->isAjax()) {

                                $this->form_validation->set_rules('Title', 'Tiêu đề của section', 'trim|required');
                                $this->form_validation->set_rules('Orders', 'Thứ tự của section', 'trim|numeric|is_natural_no_zero');
                                $this->form_validation->set_rules('Publish', 'Chế độ ẩn hiện của section', 'trim|numeric|is_natural_no_zero');
                                $this->form_validation->set_error_delimiters('<span class="error Required">', '</span>');

                                if (!$this->form_validation->run()) {

                                    $this->load->view('includes/header', $data);
                                    $this->load->view('section_edit_view');
                                    $this->load->view('includes/footer');

                                } else {

                                    if ($this->Section_model->Section_update($id)) {
                                        redirect($this->getUrl());
                                    } else {
                                        echo "fail";
                                    }
                ...
                case 'add' :
                    {
                        ...
                        if (!$this->isAjax()) {
                            ...
                            if (!$this->form_validation->run()) {
                                ...
                                $this->load->view('includes/header', $data);
                                $this->load->view('section_add_view');
                                $this->load->view('includes/footer');

                            } else {

                                if ($this->Section_model->Section_insert()) {
                                    redirect(base_url() . 'administrator/sections');
                                } else {
                                    echo "fail";
                                }
            ...
            if (isset($currentpage) && $currentpage != '') {
                ...
                $sections_list = $this->Section_model->Section_get_all($start, $show, $whereClause);
                $data['sections_list'] = $sections_list->result_array();
            }

            if (count($data['sections_list']) > 0)
                $count = $data['sections_list'][0]['count'];
            else $count = 0;
            ...
            $data['link'] = $this->Section_model->genPageLink($currentpage, 5, ceil($count / $show), $this->getUrl());
    ...
    /************************************************************************************************/
    /*********** ket thuc thao tac voi quan ly sections  */
        ...
        if ($module !== false && $module == "price") {
            if ($action !== false) {
                switch ($action) {
                    ...
                    case 'edit' :
                        {
                            $this->edit_sorting_price($cateid);
                            break;
                        }
                    default:
                        {
    ...
    public function default_sorting_price()
    {
        ...
        if (isset($currentpage) && $currentpage != '') {
            ...
            $data['category_list'] = $this->sorting_price_model->Category_get_all($start, (int)($start + $show));
        }
    ...
    public function add_sorting_price()
    {
        ...
        if (!$this->isAjax()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($this->sorting_price_model->Category_insert()) {
    ...
    public function edit_sorting_price($cateid)
    {
        $this->load->model('sorting_price_model');
        $category = $this->sorting_price_model->Category_get_by_id($cateid);
        $data['category'] = $category->row_array();
        $data['js'] = '<script>$(function() {$( "#tabs" ).tabs();});</script>';
        if (!$this->isAjax()) {
            ...
            if (!$this->form_validation->run()) {
                $this->load->view('includes/header', $data);
                $this->load->view('sorting_price_edit_view');
                $this->load->view('includes/footer');

            } else {
                if ($this->sorting_price_model->Category_update($cateid)) {
                    redirect(base_url() . "administrator/sorting/price");
                } else {
                    echo "fail";
                }
    ...
    public function subscribe()
    {
        ...
        $data['js'] = '<script>$(function() {$( "#tabs" ).tabs();});</script>';
        $data['subscribe'] = $this->subscribe_model->Get_All_Subscribe();
        $this->load->view('includes/header', $data);
    ...
    public function comment()
    {
        ...
        $this->load->view('includes/header', $data);
        $this->load->view('comment_view.php');
        $this->load->view('includes/footer');
    }
    ...
    public function sitemap()
    {
        ...
        $myfile = fopen("sitemap_index.xml", "w") or die("Unable to open file!");
        $data['sitemap_index_result'] = fwrite($myfile, $sitemap_index);
        fclose($myfile);
        // 2.	Sitemap_category.xml
        $sitemap_category = "";
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">';
        $sitemap_category .= '
			<url>
				<loc>' . base_url() . '</loc>
				<lastmod>2017-01-03T14:07:48+07:00</lastmod>
				<changefreq>daily</changefreq>
				<priority>1.0</priority>
			</url>';
        $category_product = $this->sitemap_model->get_all_category_product();
        if ($category_product) {
            foreach ($category_product as $cate_product) {
                $sitemap_category .= '
					<url>
						<loc>' . base_url($cate_product->Slug) . '.html</loc>
						<lastmod>' . date("Y-m-d") . 'T' . date("H:i:s") . '+07:00</lastmod>
        ...
        $category_news = $this->sitemap_model->get_all_category_news();
        if ($category_news) {
        ...
        $myfile = fopen("sitemap_category.xml", "w") or die("Unable to open file!");
        $data['sitemap_category_result'] = fwrite($myfile, $sitemap_category);
        fclose($myfile);
        // 3.	Sitemap_product.xml
        $sitemap_product = "";
        $sitemap_product .= '<?xml version="1.0" encoding="UTF-8"?>
			<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">';
        $products = $this->sitemap_model->get_all_product();
        if ($products) {
            foreach ($products as $prd) {
                $sitemap_product .= '
					<url>
						<loc>' . base_url($prd->Slug) . '.html</loc>
						<image:image>
						<image:loc>' . base_url() . 'resources/uploads/images/automatic/san-pham/' . $prd->ImageURL . '</image:loc>
        ...
        $sitemap_product .= '</urlset>';
        $myfile = fopen("sitemap_product.xml", "w") or die("Unable to open file!");
        $data['sitemap_product_result'] = fwrite($myfile, $sitemap_product);
        fclose($myfile);
        // 4.	Sitemap_news.xml
        $sitemap_news = "";
        $sitemap_news .= '<?xml version="1.0" encoding="UTF-8"?>
			<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">';
        $news = $this->sitemap_model->get_all_news();
        if ($news) {
            foreach ($news as $news_item) {
                $sitemap_news .= '
					<url>
						<loc>' . base_url($news_item->Slug) . '.html</loc>
						<image:image>
						<image:loc>' . base_url() . 'resources/uploads/images/automatic/tin-tuc/' . $news_item->ImageURL . '</image:loc>
						<image:license>' . base_url() . '</image:license>
						<image:family_friendly>yes</image:family_friendly>
						</image:image>
						<lastmod>' . date("Y-m-d") . 'T' . date("H:i:s") . '+07:00</lastmod>
        ...
        $sitemap_news .= '</urlset>';
        $myfile = fopen("sitemap_news.xml", "w") or die("Unable to open file!");
        $data['sitemap_news_result'] = fwrite($myfile, $sitemap_news);
        fclose($myfile);
        // 5.	Sitemap_tag.xml
        // 6.	Sitemap_image.xml
        $sitemap_product_images = "";
        $sitemap_product_images .= '<?xml version="1.0" encoding="UTF-8"?>
            ...
            foreach ($products as $prd) {
                $sitemap_product_images .= '
					<url>
						...
						<image:image>
							<image:loc>' . base_url() . 'resources/uploads/images/automatic/san-pham/' . $prd->ImageURL . '</image:loc>
							<image:title>' . $prd->ImageTitle . '</image:title>
							<image:caption>' . $prd->ImageAlt . '</image:caption>
        ...
        $sitemap_product_images .= '</urlset>';
        $myfile = fopen("sitemap_image.xml", "w") or die("Unable to open file!");
        $data['sitemap_image_result'] = fwrite($myfile, $sitemap_product_images);
        fclose($myfile);
        $data['js'] = '<script>$(function() {$( "#tabs" ).tabs();});</script>';
        $this->load->view('includes/header', $data);
