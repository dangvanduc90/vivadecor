<div class="grid_12">
	<div class="header-content">Danh sách chứng chỉ</div>
    <div class="clear"></div>
    <div class="command">
    ...
    <div class="wrap-main-body">
        ...
        <div style="padding:1px;">
        	<table class="table-view">
	        	...
	        	<tbody>
              <?php if(count($certificate_list) > 0 ) {?>
  	        		<?php $i = 1; foreach ($certificate_list as $key) :?>
  		        		<tr id="banner<?= $key['Id']?>">
                    ...
                    <td align="center" style="width:60px;text-align:center;"><input type="text"  id="txtOrders<?= $key['Id']?>" style="width: 60px; text-align:right;" value="<?= $key['Orders']?>"  onchange="UpdateOrders(<?= $key['Id']?>,this);"/></td>
  		        			...
  		        			<td align="center" style="width:120px;text-align:center;"><a href="<?= base_url()?>administrator/certificate/edit/<?= $key['Id']?>">Chỉnh sửa</a> | <a href="javascript:void(0);" onclick="showconfirm(<?= $key['Id']?>,'Xác thực xóa banner','Bạn có chắc muốn xóa banner này?','delete');">xóa</a></td>
  		        		</tr>
  		        	<?php $i++; endforeach;?>
              <?php }else {?>
                  <tr>
                    <td colspan="6">Chưa có chúng chỉ nào cả</td>
                  </tr>
              <?php }?>
	        	</tbody>
...
<script type="text/javascript">
 //<![CDATA[
    ...
    function UpdatePublishStatus(id) {
      ...
      $.ajax( {
            type: "POST",
            url: "<?= base_url()?>ajaxhandle/admin_chungchi_ajaxhandler/updatePublish",
    ...
    function UpdateOrders(id,element) {
      ...
      if(!isNaN(orders)) {
        $.ajax( {
              type: "POST",
              url: "<?= base_url()?>ajaxhandle/admin_chungchi_ajaxhandler/updateOrders",
    ...
    function Delete_banner(id) {
      $.ajax( {
            type: "POST",
            url: "<?= base_url()?>ajaxhandle/admin_chungchi_ajaxhandler/delete",
            data: { id: id},
            dataType: "json",
            success: function(data) {
              if(data == 0) {
