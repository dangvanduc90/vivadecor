<div class="grid_12">
	<div class="header-content">Quản lý menu phụ</div>
    <div class="clear"></div>
    <div class="command">
  	        	...
  	        	<tbody>
    	        		<?php $i = 1; foreach ($menu_list as $key) :?>
    		        		<tr id="menu<?= $key['MenuID']?>">
    		        			...
    		        			<td><a id="Title<?= $key['MenuID']?>" href="<?= base_url()?>administrator/menuhot/edit/<?= $key['MenuID']?>"><?= $key['Title']?></a></td>
...
<script type="text/javascript">
 //<![CDATA[
  function UpdateNewTabStatus(id) {
    ...
    $.ajax( {
          type: "POST",
          url: "<?= base_url()?>ajaxhandle/admin_menu_hot_ajaxhandler/Update_NewTab",
  ...
  function UpdatePublishStatus(id) {
    ...
    $.ajax( {
          type: "POST",
          url: "<?= base_url()?>ajaxhandle/admin_menu_hot_ajaxhandler/Update_Publish",
  ...
  function UpdateIsClickStatus(id) {
    ...
    $.ajax( {
          type: "POST",
          url: "<?= base_url()?>ajaxhandle/admin_menu_hot_ajaxhandler/Update_IsClick",
  ...
  function UpdateOrders(id,element) {
        ...
        if(!isNaN(orders)) {
            $.ajax( {
                  type: "POST",
                  url: "<?= base_url()?>ajaxhandle/admin_menu_hot_ajaxhandler/Update_Orders",
    ...
    function Delete_Menu(id) {
        $.ajax( {
            type: "POST",
            url: "<?= base_url()?>ajaxhandle/admin_ajaxhandler/Delete_menu_hot",
            data: { id: id},
            dataType: "json",
            success: function(data) {
                if(data == 0){
