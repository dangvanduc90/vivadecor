<div class="grid_12">
	<div class="header-content">Danh sách tag sản phẩm</div>
    <div class="clear"></div>
    <div class="command">
              ...
              <?php if(count($tags_list) > 0 ) {?>
  	        		<?php $i = 1; foreach ($tags_list as $key) :?>
  		        		<tr id="tags<?= $key['TagsID']?>">
  		        			...
  		        			<td align="center" style="text-align:left;"><a id="txtTagsTitle<?= $key['TagsID']?>" href="<?= base_url()?>administrator/productstags/edit/<?= $key['TagsID']?>"><?= $key['Title']?></a></td>
                    ...
                    <td align="center" style="width:120px;text-align:center;"><a href="<?= base_url()?>administrator/productstags/edit/<?= $key['TagsID']?>">Chỉnh sửa</a> | <a href="javascript:voi(0);" onclick="showconfirm(<?= $key['TagsID']?>,'Xác thực xóa tags','Bạn có muốn xóa tags này','delete')">xóa</a></td>
  		        		</tr>
  		        	<?php $i++; endforeach;?>
              <?php }else {?>
                  <tr>
    ...
    <div class="clear"></div>
    <div class="command">
        <span><a href="<?= base_url()?>administrator/tag/add">Thêm mới</a></span>
    </div>
</div>
...
<script type="text/javascript">
 //<![CDATA[
  ...
  function UpdatePublishStatus(id) {
    ...
    $.ajax( {
          type: "POST",
          url: "<?= base_url()?>ajaxhandle/admin_products_tags_ajaxhandler/Update_Publish",
  ...
  function UpdateOrders(id,element) {
        ...
        if(!isNaN(orders)) {
            $.ajax( {
                  type: "POST",
                  url: "<?= base_url()?>ajaxhandle/admin_products_tags_ajaxhandler/Update_Orders",
    ...
    function Delete_Tags(id) {
        $.ajax( {
            type: "POST",
            url: "<?= base_url()?>ajaxhandle/admin_ajaxhandler/Delete_products_tags",
            data: { id: id},
            dataType: "json",
            success: function(data) {
                if(data == 0){
