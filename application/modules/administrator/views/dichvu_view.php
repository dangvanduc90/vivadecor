        ...
        <div style="padding:1px;">
          <table class="table-view">
            ...
            <tbody>
              ...
              <?php $i = 1; foreach ($dichvu_list as $key) :?>
                <tr id="dichvu<?= $key['Id']?>">
                  <td align="center" style="width:20px;text-align:center;"><?= $i?></td>
                  <td align="center" style="width:60px;text-align:center;"><img src="<?= base_url()?>resources/uploads/images/thumbs/<?= $key['ImageUrl']?>" height="60" width="100"/></td>
                  <td><a id="txtPrdName<?= $key['Id']?>" href="<?= base_url()?>administrator/dichvu/edit/<?= $key['Id']?>"><?= $key['Title']?></a></td>
                  ...
                  <td align="center" style="width:120px;text-align:center;"><a href="<?= base_url()?>administrator/dichvu/edit/<?= $key['Id']?>">Chỉnh sửa</a> | <a href="javascript:void(0);" onclick="showconfirm(<?= $key['Id']?>,'Xác thực xóa dữ liệu','Bạn có chắc muốn xóa?','delete');">xóa</a></td>
                </tr>
              <?php $i++; endforeach;?>
              <?php }else {?>
                  <tr>
                    <td colspan="8">Chưa có dịch vụ nào cả</td>
                  </tr>
              <?php }?>
            </tbody>
...
<script type="text/javascript">
 //<![CDATA[
  function UpdatePublishStatus(id) {
    ...
    $.ajax( {
          type: "POST",
          url: "<?= base_url()?>ajaxhandle/admin_dichvu_ajaxhandler/updatePublish",
  ...
  function UpdateaaaHeight(id) {
    ...
    $.ajax( {
          type: "POST",
          url: "<?= base_url()?>ajaxhandle/admin_ajaxhandler/Update_dichvu_height",
  ...
  function UpdateWidth(id,element) {
    ...
    if(!isNaN(width)) {
      $.ajax( {
            type: "POST",
            url: "<?= base_url()?>ajaxhandle/admin_ajaxhandler/Update_dichvu_width",
  ...
  function UpdateOrders(id,element) {
    ...
    if(!isNaN(orders)) {
      $.ajax( {
            type: "POST",
            url: "<?= base_url()?>ajaxhandle/admin_dichvu_ajaxhandler/updateOrders",
  ...
  function showconfirm(id,title,message,action){
    ...
    function Delete_Ads(id) {
        $.ajax( {
            type: "POST",
            url: "<?= base_url()?>ajaxhandle/admin_dichvu_ajaxhandler/delete",
            data: { id: id},
            dataType: "json",
            success: function(data) {
                if(data == 0){
