<div class="grid_12">
	<form method="post" action="<?= current_url();?>" name="form-edit" id="form-edit" accept-charset="utf-8">
		<div class="header-content">Chỉnh sửa logo</div>
		<div class="clear"></div>
		<div class="command">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								...
								<input name="Title" type="text" value="<?= $logo['Title']?>" maxlength="500" id="logoTitle" class="TextInput">
								<span class="tooltip">
							...
							<td>
								...
								<table id="tblUpload" cellspacing="0" cellpadding="0" width="100%">
									<tbody>
										...
										<tr>
											<td>
												<img id="img_upload" src="<?= base_url()?>resources/uploads/images/automatic/logo/thumbs/<?= $logo['ImageURL']?>">
												<input type="hidden" name="ImageURL" id="ImageURL" value="<?= $logo['ImageURL']?>">
											</td>
										</tr>
										<tr>
											<td>
						...
						<tr>
							...
							<td>
								...
								<textarea name="Description" rows="2" cols="20" id="logoDescription" style="height:96px;width:400px;"><?= $logo['Description']?></textarea>
								<span class="tooltip">
									<span class="tooltipContent">
				...
				<table class="admintable" width="100%">
					<tbody>
						...
						<tr>
							...
							<td><span class="lbl-key"><?php if($logo['CreatedBy'] =='') echo '<span class="lbl-key-nan">Không xác định</span>'; else echo '<span class="lbl-key">'.$logo['CreatedBy'].'</span>';?></span>
								<span class="tooltip">
									<span class="tooltipContent">
						...
						<tr>
							...
							<td><span class="lbl-key"><?= $logo['CreatedDate']?></span>
								<span class="tooltip">
									<span class="tooltipContent">
						...
						<tr>
							...
							<td><?php if($logo['ModifiedBy'] =='') echo '<span class="lbl-key">Không xác định</span>'; else echo '<span class="lbl-key">'.$logo['ModifiedBy'].'</span>';?>
								<span class="tooltip">
									<span class="tooltipContent">
						...
						<tr>
							...
							<td><?php if($logo['ModifiedDate'] =='') echo '<span class="lbl-key-nan">Chưa từng sửa</span>'; else echo '<span class="lbl-key">'.$logo['ModifiedDate'].'</span>';?>
								<span class="tooltip">
									<span class="tooltipContent">
...
<script type="text/javascript">
 //<![CDATA[
	...
	function upload_images_scroll() {
		 $.ajaxFileUpload({
			url         :"<?= base_url()?>ajaxhandle/admin_ajaxhandler/Logo_Upload/userfilescroll",
			secureuri      :false,
			fileElementId  :'userfilescroll',
			dataType    : 'json',
			data        : {},
			success  : function (data) {
				if(data.url != "" && data.msg == "" && data.imgname != "") {
					$('#img_upload_scroll').attr('src',data.url);
					$('#ImageURLScroll').attr('value',data.imgname);
				}
				else if(data.msg != "")
					notice(data.msg);
			},
	...
	function upload_images_footer() {
		$.ajaxFileUpload({
			url         :"<?= base_url()?>ajaxhandle/admin_ajaxhandler/Logo_Upload/userfilefooter",
			secureuri      :false,
			fileElementId  :'userfilefooter',
			dataType    : 'json',
			data        : {},
			success  : function (data) {
				if(data.url != "" && data.msg == "" && data.imgname != "") {
					$('#img_upload_footer').attr('src',data.url);
					$('#ImageURLFooter').attr('value',data.imgname);
				}
				else if(data.msg != "")
					notice(data.msg);
			},
				...
				'Yes'   : {
					...
					'action': function(){
						if(action == 'save') {
							...
							if (id == 1) {
								upload_images_scroll();
							}else if(id == 2){
								upload_images_footer();
							} else upload_images();
							$('body').css('overflow','auto');
						}
					}
