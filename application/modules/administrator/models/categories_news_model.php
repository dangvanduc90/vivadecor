	...
	class Categories_news_model extends Cms_Base_model {
		...
		public function convert() {
			...
			foreach ($arr as $key => $value) {
				...
				// $this->db->where('NewsID', $value->object_id);
				// $this -> db -> update('news',$data);

				$this -> db -> update('news',$data,array('NewsID' => $value->object_id));
		...
		public function arrID()	{
			$data = array("7");
			$this->db->select('object_id');
			$this->db->where_in("term_taxonomy_id", $data);
			$query = $this->db->get("wp_term_relationships");
