<?php
	class News_model extends Cms_Base_model {
		...
		private function add_news_tags($tag,$tag_slug){
			$data_input = array('Title' => $tag, 'Slug' => $tag_slug,'Publish' => 1,'Orders' => 999);
			$this -> db -> insert('newstags',$data_input);
		...
		private function check_exists_tags($tag){
            ...
            if ($query -> num_rows() > 0){
            	...
            	return $this -> add_news_tags($tag,$tag_slug);
            }
		}
		...
		private function check_field_input() {
			...
			$this -> input -> post('newsImageAlt') ? $newsImageAlt = $this -> input -> post('newsImageAlt') : $newsImageAlt = substr($newsMainImage, 0 , (strrpos($newsMainImage, ".")));
			...
			$this -> input -> post('newsTags') ? $newsTags = $this -> createTag($this -> input -> post('newsTags')) : $newsTags = "";
