<?php
	class News_tags_model extends Cms_Base_model {
        ...
        public function Tags_get_all($limit_start=false,$limit_show=false,$whereClause = ' 1 = 1 ') {
			...
			$selectClause = " select (select count(TagsID) from newstags where "
								.$whereClause. "
								) as count, TagsID,Title,Orders,CAST(Publish as UNSIGNED INT) as Publish from newstags 
		...
		public function Tags_get_except_by_id($id = false) {
			$selectClause = " TagsID,Title from newstags where TagsID not like ".$id;
		...
		private function get_newest_id() {
			...
			$this -> db -> where('TABLE_NAME', 'newstags');
		...
		public function Tags_update($id) {
			...
			return $this -> db -> update('newstags',$data,array('TagsID' => $id));
		}

		public function Tags_delete($id = false) {
			if($id !== false) {
				$this -> db -> delete('newstags',array('TagsID' => $id));
