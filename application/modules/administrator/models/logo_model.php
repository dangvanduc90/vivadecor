<?php
	class Logo_model extends Cms_Base_model {
		...
		public function Logo_get_all($limit_start=false,$limit_show=false) {
			...
			$selectClause = " select (select count(OrdersID) from orders) as count, LogosID,ImageURL,ImageURLScroll,ImageURLFooter,
		...
		public function Logo_get_by_id($id = false) {
			...
			if($id !== false) {
				$selectClause = " select LogosID,ImageURL,ImageURLScroll,ImageURLFooter,Title,Description,CAST(IsMain as UNSIGNED INT) as IsMain,
		...
		private function Check_input_fields() {
			...
			$this -> input -> post('ImageURLScroll') ? $ImageURLScroll = $this -> input -> post('ImageURLScroll') : $ImageURLScroll = "";
			$this -> input -> post('ImageURLFooter') ? $ImageURLFooter = $this -> input -> post('ImageURLFooter') : $ImageURLFooter = "";
		...
		public function Logo_insert() {
			...
			$data = array(
						...
						'ImageURLScroll' => $receivedata['ImageURLScroll'],
						'ImageURLFooter' => $receivedata['ImageURLFooter'],
