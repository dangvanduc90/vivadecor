<?php
	class Categories_product_model extends Cms_Base_model {
		...
		public function Category_get_all($limit_start=false,$limit_show=false) {
			$this -> db -> select(' cate.CategoriesProductsID,cate.Title,cate.Orders,CAST(cate.Publish as UNSIGNED INT) as Publish,CAST(cate.IsTop as UNSIGNED INT) as IsTop,cate.ParentID, (select count(CategoriesProductsID) 
		...
		public function Category_get_by_id($CateID = false) {
			if($CateID !== false) {
				$selectClause = " cate.CategoriesProductsID,cate.Title,cate.Title_en,cate.Title_fr,cate.Description,cate.Body,cate.ImagesURL,cate.ImagesTitle,cate.ImagesAlt,
								  cate.ParentID,CAST(cate.Publish as UNSIGNED INT) as Publish,CAST(cate.IsTop as UNSIGNED INT) as IsTop,cate.Orders,cate.SEOTitle,cate.SEOKeyword,cate.SEODescription,cate.SortingBrand,cate.SortingPrice,cate.SortingRes,cate.SortingChannel,cate.Tags,
		...
		private function check_input_field() {
        	...
        	*/
            ...
            $this -> input -> post('cateTags') ? $cateTags = $this -> createTag($this -> input -> post('cateTags')) : $cateTags = "";
