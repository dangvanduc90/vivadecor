	...
	class Sorting_price_model extends Cms_Base_model {
		...
		private function Categories($parentid = 0,$array = array(),$space = '|-----',$trees = array()){ 
			...
			for ($i=0; $i < count($temp); $i++) { 
				...
				$trees[] = $temp[$i];
				$trees = $this -> Categories($temp[$i]['SortingPriceID'],$array,$space.'|-----',$trees);
			}
		...
		public function Category_get_all($limit_start=false,$limit_show=false) {
			$this -> db -> select(' cate.SortingPriceID,cate.Title,cate.Orders,cate.Publish,cate.IsTop,cate.ParentID, (select count(SortingPriceID) 
										from sortingprice) as count from sortingprice as cate');
		...
		public function Category_get_by_id($CateID = false) {
			if($CateID !== false) {
				$selectClause = " cate.SortingPriceID,cate.Title,cate.Title_en,cate.Title_fr,cate.Description,cate.Body,cate.ImagesURL,cate.ImagesTitle,cate.ImagesAlt,
				...
				$this -> db -> where("cate.SortingPriceID = ".$CateID.";");
		...
		public function Category_get_except_by_id($CateID = false) {
			$selectClause = "* from sortingprice where SortingPriceID not like ".$CateID;
			$this -> db -> select($selectClause);
			$result = $this -> db -> get();
		...
		public function Category_get_by_slug($Slug = false) {
			$result = $this -> db -> get_where('sortingprice',array('Slug' => $CateID));
		...
		private function check_input_field() {
        	...
        	*/
            ...
            $this -> input -> post('cateMetaDesc') ? $SEODescription = $this -> input -> post('cateMetaDesc') : $SEODescription = $this -> input -> post('cateDescription');
        	...
        	$data = array(
						...
						'Publish' => $Publish,
						'IsTop' => $IsTop,
						'ImagesURL' => 'danh-muc-do-phan-giai/' . $cateImagesURL,
						'Slug' => $Slug,
						'SEOTitle' => $SEOTitle,
		...
		public function Category_update($CateID) {
			...
			$this -> db -> update('sortingprice',$data,array('SortingPriceID' => $CateID));
