	...
	class Sorting_channel_model extends Cms_Base_model {
		...
		private function Categories($parentid = 0,$array = array(),$space = '|-----',$trees = array()){ 
			...
			for ($i=0; $i < count($temp); $i++) { 
				...
				$trees[] = $temp[$i];
				$trees = $this -> Categories($temp[$i]['SortingChannelID'],$array,$space.'|-----',$trees);
			}
		...
		public function Category_get_all($limit_start=false,$limit_show=false) {
			$this -> db -> select(' cate.SortingChannelID,cate.Title,cate.Orders,cate.Publish,cate.IsTop,cate.ParentID, (select count(SortingChannelID) 
										from sortingchannel) as count from sortingchannel as cate');
		...
		public function Category_get_all_for_select_box() {
			$this -> db -> select("SortingChannelID,Title,ParentID");
		...
		public function Category_get_by_id($CateID = false) {
			if($CateID !== false) {
				$selectClause = " cate.SortingChannelID,cate.Title,cate.Title_en,cate.Title_fr,cate.Description,cate.Body,cate.ImagesURL,cate.ImagesTitle,cate.ImagesAlt,
				...
				$this -> db -> from("sortingchannel as cate");

				$this -> db -> where("cate.SortingChannelID = ".$CateID.";");
		...
		public function Category_get_except_by_id($CateID = false) {
			$selectClause = "* from sortingchannel where SortingChannelID not like ".$CateID;
			$this -> db -> select($selectClause);
			$result = $this -> db -> get();
		...
		public function Category_get_by_slug($Slug = false) {
			$result = $this -> db -> get_where('sortingchannel',array('Slug' => $CateID));
		...
		public function Category_insert() {
			...
			$this -> db -> insert('sortingchannel',$data);
		...
		public function Category_update($CateID) {
			...
			$this -> db -> update('sortingchannel',$data,array('SortingChannelID' => $CateID));
		...
		private function check_con($idCha = false) {
			$selectClause = " select SortingChannelID from sortingchannel where ParentID = ? ";
		...
		private function dequy_xoa($idCha = false) {
			...
			if($idCha !== false) {
				...
				$deleteClause = " delete from sortingchannel where SortingChannelID = ? ";
