<?php
	class Banner_model extends Cms_Base_model {
		...
		public function Banner_get_all($limit_start=false,$limit_show=false,$whereClause = ' 1 = 1 ') {

			$selectClause = " select (select count(BannerID) from banner where "
								.$whereClause. "
								) as count, BannerID,Title,ImageURL,Orders,CAST(Publish as UNSIGNED INT) as Publish from banner
		...
		public function Banner_get_by_id($id = false) {

			$selectClause = " select BannerID,Title,Body,Title_en,Body_en,ImageURL,Orders,CAST(Publish as UNSIGNED INT) as Publish,Link,
		...
		public function Banner_update($id = false) {
		...
		public function  Banner_update_orders() {
		...
		public function Banner_update_publish() {
			$id = $this -> input -> post('id');
			$this -> input -> post('publish') ? $publish = 1 : $publish = 0;
			if($id !== false && is_numeric($id)) {
		...
		public function Banner_delete($id = false) {
			if($id !== false) {
				$this -> db -> delete('banner',array('BannerID' => $id));
